...
export function createScene(
  ...
  const runtime = createPrismRuntime(gpu, output, label, { lightMeshLayout: LOW_LIGHT_MESH_LAYOUT });
  setRuntimeControls(runtime, {
    ...DEFAULT_PRISM_CONTROLS,
    beamWidth: 0.012,
    spectralDispersion: { base: 1.245, strength: 0.06 },
    postprocess: { bloomStrength: 0.08, bloomThreshold: 0.8, bloomRadius: 0.25 },
    glass: { ...DEFAULT_PRISM_CONTROLS.glass, reflection: {
      ...DEFAULT_PRISM_CONTROLS.glass.reflection,
      dark: { reflectionStrength: 2.14, environmentExposure: 2.3 },
    } },
  });
  return { runtime, pipeline: createDarkPipeline(runtime, { quality: "low" }) };
}
